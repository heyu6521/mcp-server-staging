...
export class ConfirmationService {
  ...
  consume(
    ...
    if (!raw || !sig)
      throw new AppError("invalid_input", "Invalid confirmation token", 400);
    const actual = crypto
      .createHmac("sha256", this.secret)
      .update(raw)
      .digest();
    ...
    if (
      ...
      throw new AppError(
        "forbidden",
        "Confirmation token is expired or mismatched",
        403,
      );
    ...
    if (JSON.stringify(payload.summary) !== JSON.stringify(expected))
      throw new AppError(
        "conflict",
        "Operation state changed since confirmation was prepared",
        409,
      );
