...
export function createApp(config: RuntimeConfig, provider: GitPlatformProvider) {
  ...
  const allBucket: Bucket = { minute: 0, count: 0 }; const writeBucket: Bucket = { minute: 0, count: 0 }; let inFlight = 0;
  app.use((req, res, next) => {
    const requestId = crypto.randomUUID(); res.setHeader('x-request-id', requestId);
    if (config.authMode === 'bearer' && !bearerOk(req.header('authorization'), config.bearerToken)) { res.status(401).json({ error: 'unauthorized', requestId }); return; }
    if (!consume(allBucket, config.rateLimitPerMinute)) { res.status(429).json({ error: 'rate_limited', requestId }); return; }
    const body = req.body as { method?: string; params?: { name?: string } } | undefined;
    const isWrite = body?.method === 'tools/call' && typeof body.params?.name === 'string' && WRITE_TOOLS.has(body.params.name);
    if (isWrite && !consume(writeBucket, config.writeRateLimitPerMinute)) { res.status(429).json({ error: 'write_rate_limited', requestId }); return; }
    if (req.path === '/mcp') {
      if (inFlight >= config.maxConcurrency) { res.status(503).json({ error: 'concurrency_limit', requestId }); return; }
      inFlight += 1; let released = false; const release = () => { if (!released) { released = true; inFlight -= 1; } }; res.once('finish', release); res.once('close', release);
    }
    log('info', 'http_request', { requestId, method: req.method, path: req.path }); next();
  });
  app.get('/healthz', (_req, res) => res.json({ status: 'ok' }));
  app.get('/readyz', async (_req, res) => {
    try { await provider.getMe(); res.json({ status: 'ready' }); }
